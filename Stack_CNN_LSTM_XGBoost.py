...
class StackCNNLSTMXGBoostPredictor:
    """
    基于CNN、LSTM和XGBoost的Stack4集成预测器
    用于预测电力负载
    """
    ...
    def _build_meta_learner(self, merged):
        """构建元学习器"""
        x = Dense(512)(merged)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
    ...
    def _build_model(self):
        ...
        # 合并三个分支
        merged = Concatenate()([cnn_output, lstm_output, xgb_output])

        # 元学习器
        x = self._build_meta_learner(merged)
        outputs = Dense(self.forecast_points)(x)

        # 构建模型
        model = Model(
            inputs=[cnn_input, lstm_input, xgb_input],
            outputs=outputs
        )
        model.compile(
    ...
    def predict(self, df: pd.DataFrame, save_dir: Optional[str] = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logging.info(f"Training Stack4 with input shapes: CNN={X_cnn.shape}, "
                         f"LSTM={X_lstm.shape}, XGB={X_xgb.shape}, "
    ...
    def _plot_predictions(self, historical_df: pd.DataFrame,
        ...
        ax1.plot(predictions_df['timestamp'], predictions_df['predicted_load'],
                 label='Stack4 Predicted Load', color='red', alpha=0.6)
        ax1.set_title(f'{self.forecast_hours}-hour Stack4 Load Forecast')
        ax1.set_xlabel('Time')
        ...
        if save_dir:
            ...
            plt.savefig(os.path.join(save_dir, f'stack4_prediction.png'),
                        dpi=300, bbox_inches='tight')
            logging.info(f"Stack4 prediction plot saved in {save_dir}")
        else:
            plt.show()
    ...
    def _print_prediction_summary(self, predictions: np.ndarray) -> None:
        """打印预测结果统计信息"""
        logging.info("Stack4 Prediction Summary:")
    ...
    def predict_with_split(self, X_train, y_train, X_test, test_df: pd.DataFrame, save_dir: Optional[str] = None) -> \
        ...
        try:
            ...
            # 训练模型
            logging.info("Training Stack4 (CNN+LSTM+XGBoost) model with split data...")
            history = self.model.fit(
    ...
    def _plot_predictions_with_split(self, test_df: pd.DataFrame,
        ...
        plt.plot(predictions_df['timestamp'], predictions_df['predicted_load'],
                 label='Stack4 Predicted Load', color='red', alpha=0.6)
        plt.title('Stack4 (CNN+LSTM+XGBoost) Load Forecast with Train-Test Split')
        plt.xlabel('Time')
        plt.ylabel('Load (MW)')
        plt.legend()
        ...
        if save_dir:
            os.makedirs(save_dir, exist_ok=True)
            plt.savefig(os.path.join(save_dir, f'stack4_prediction_split.png'),
                        dpi=300, bbox_inches='tight')
            logging.info(f"Stack4 (CNN+LSTM+XGBoost) prediction plot saved in {save_dir}")
        else:
            plt.show()
