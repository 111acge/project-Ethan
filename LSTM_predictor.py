...
class LSTMPredictor:
    ...
    def _build_model(self, input_shape: Tuple[int, int]) -> None:
        ...
        """
        with tf.device('/device:GPU:0'):  # 明确指定使用GPU
            ...
            logging.info("LSTM model structure:")
            self.model.summary()
    ...
    def predict(self, df: pd.DataFrame, save_dir: Optional[str] = None) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            logging.info(f"Training LSTM with input shape: {X.shape}, output shape: {y.shape}")
    ...
    def _plot_predictions(self, historical_df: pd.DataFrame,
        ...
        if save_dir:
            ...
            logging.info(f"LSTM prediction plot saved in {save_dir}")
        else:
            plt.show()
    ...
    def _print_prediction_summary(self, predictions: np.ndarray) -> None:
        ...
        打印预测结果的统计信息
        """
        logging.info("LSTM Prediction Summary:")
    ...
    def predict_with_split(self, X_train, y_train, X_test, test_df: pd.DataFrame, save_dir: Optional[str] = None) -> \
        ...
        try:
            ...
            # 训练模型
            logging.info("Training LSTM model with split data...")
            history = self.model.fit(
                X_train, y_train,
                epochs=100,
                batch_size=128,
    ...
    def _plot_predictions_with_split(self, test_df: pd.DataFrame,
        ...
        plt.title(f'LSTM Load Forecast with Train-Test Split')
        plt.xlabel('Time')
        plt.ylabel('Load (MW)')
        plt.legend()
        plt.grid(True)
        ...
        if save_dir:
            os.makedirs(save_dir, exist_ok=True)
            plt.savefig(os.path.join(save_dir, f'lstm_prediction_split.png'),
                        dpi=300, bbox_inches='tight')
            logging.info(f"LSTM prediction plot saved in {save_dir}")
        else:
            plt.show()
